...
struct QuadraticProbingHashTable{
    ...
    QuadraticProbingHashTable(unsigned long Size) : table(Size, ""), size(Size) {}

    unsigned long hash(const string& name, int tmp){
        return(djb2hash((unsigned char*)name.c_str()) + tmp * tmp) % size;
    }
...
// QuadraticProbingHashTable HashTable(HashTableSize);
